<template>
    <el-radio-group  v-model="chartConfig.option.modelValue" v-on="getEvents" :style="getStyle">
      <template v-for="(item,index) in chartConfig.option.listOptions" :key="index">
        <el-radio :label="item.value"  v-bind="chartConfig.option.attrs">{{ item.label }}</el-radio>
      </template>
    </el-radio-group>
</template>
...
import { option as configOption,EventNameList, Events } from './config'
