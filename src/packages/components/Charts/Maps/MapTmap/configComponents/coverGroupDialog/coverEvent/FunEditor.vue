  ...
  <el-container>
    <el-main style="padding: 0">
      <el-tabs v-model="tabsActiveName" type="border-card" class="demo-tabs">
        <el-tab-pane
          v-for="(event, index) in ControlGroupEvents"
        ...
        >
          ...
          <p class="go-pl-3">
            ...
            <span class="func-keyNameWord"
              >{{ event.key }}({{ event.paramsName }})&nbsp;&nbsp;{</span
            >
          </p>
          <!-- 编辑主体 -->
          <monaco-editor
...
import { MapBaseEvent } from "@/enums/eventEnum";
...
let tabsActiveName = ref(MapBaseEvent.ON_CLICK);
...
const saveEvents = () => {
  ...
  if (Object.values(props.baseEvent).join("").trim() === "") {
    ...
    props.targetData.events.codeEvents.baseEvent = {
      [MapBaseEvent.ON_CLICK]: undefined,
      [MapBaseEvent.ON_DBL_CLICK]: undefined,
      [MapBaseEvent.ON_MOUSE_OVER]: undefined,
      [MapBaseEvent.ON_MOUSE_OUT]: undefined,
    };
  } else {
    props.targetData.events.codeEvents.baseEvent = { ...props.baseEvent };
  }
  emit("close");
};
