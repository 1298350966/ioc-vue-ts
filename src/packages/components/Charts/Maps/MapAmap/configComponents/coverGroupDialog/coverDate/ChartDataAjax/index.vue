    ...
    <el-card shadow="never" class="n-card-shallow" style="padding: 0">
      <setting-item-box name="请求配置">
        <setting-item name="类型">
          <el-tag style="border-radius: 5px">
            {{
...
const sendHandle = async () => {
  ...
  try {
    ...
    if (res) {
      ...
      setDataKeyList( targetData.value.data)
...
//收集数组数据字段
function setDataKeyList(data){
  if(Array.isArray(data) && data[0]){
    targetData.value.dataKeyList = Object.keys(data[0]).map((key)=>{
      return {
        key:key,
        type: typeof data[0][key]
      }
    })
    console.log(`output->`,targetData.value.dataKeyList)
  }
}
// 颜色
