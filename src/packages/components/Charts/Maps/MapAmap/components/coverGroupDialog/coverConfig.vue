...
const LabelMarkerConfig = defineAsyncComponent(
  () => import("./coverConfig/LabelMarkerConfig.vue")
);
const ElasticMarkerConfig = defineAsyncComponent(
  () => import("./coverConfig/ElasticMarkerConfig.vue")
);
...
function typeChange(type) {
  switch (type) {
    ...
    case coverEnum.CircleMarker:
      ...
      break;
    case coverEnum.LabelMarker:
      props.config.options = cloneDeep(LabelMarkerOptions);
      props.config.data = cloneDeep(MarkerData);
      props.config.dataMapping = cloneDeep(MarkerDataMapping);
      commentName.value = LabelMarkerConfig;
      break;
    case coverEnum.ElasticMarker:
      props.config.options = cloneDeep(ElasticMarkerOptions);
      props.config.data = cloneDeep(MarkerData);
      props.config.dataMapping = cloneDeep(MarkerDataMapping);
      commentName.value = ElasticMarkerConfig;
      break;
    // case coverEnum.MassMarks:
    //   props.config.options = cloneDeep(ElasticMarkerOptions);
    //   props.config.data = cloneDeep(MarkerData);
    //   props.config.dataMapping = cloneDeep(MarkerDataMapping);
    //   commentName.value = ElasticMarkerConfig;
