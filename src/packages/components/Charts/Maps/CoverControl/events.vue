<template>
  <el-collapse>
    <el-collapse-item title="组件事件">
      <el-card class="collapse-show-box" shadow="never">
        ...
        <div v-for="event in Events" :key="event.key">
          ...
          <p class="go-ml-4">
            <h-code :code="(componentEvents || {})[event.key] || ''" language="typescript"></h-code>
          </p>
          <p>}<span>,</span></p>
        </div>
      </el-card>
    ...
    <!-- 弹窗 -->
    <dragDialog custom-class="FunEditorDialog" v-model="showModal" title="组件事件编辑器">
      <FunEditor ref="funEditorRef" :baseEvent="componentEvents" :eventsConfig="Events" @close="closeEvents">
      </FunEditor>
      <template #footer>
        <el-space>
...
import FunEditor from "@/packages/common/FunEditor.vue"
import { option, Events} from "./config";
