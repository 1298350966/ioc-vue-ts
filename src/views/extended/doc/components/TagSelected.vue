...
<script>
    export default {
        ...
        props: {
            ...
            "classificationId":{required: true}, // 文件分类
        },
        watch: {
            filterText(val) {
        ...
        methods: {
            ...
            listDicCatalog() {
                ...
                let param = {source:_this.source,classificationId:_this.classificationId};
                if (!_this.KHUtils.isNull(_this.tenantId)) {
                    param.tenantId = _this.tenantId
                }
                _this.dicCatalogData = [];
                _this.$Get(_this.khConfig.api.listTag, param)
                    .then(res => {
            ...
            selectDicCatalog(data) {
                ...
                if (data.level !== 3) {
                    ...
                    _this.$message.warning("此为三级标签 ,不能作为上级标签选择");
                    _this.display = 'block'
                }
            },
            // 显示标签的编辑界面
            ...
            saveDicCatalog() {
                    ...
                    if (valid) {
                        _this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            if (_this.KHUtils.isNull(_this.dicCatalogForm.id)) {
                                url = _this.khConfig.api.addTag;
                            } else {
                                url = _this.khConfig.api.updateTag;
                            }
                ...
                function convertDicCatalogForm2Array(_form, _this) {
                    ...
                    if (!_this.KHUtils.isNull(_this.classificationId)) {
                        paramArray.push("classificationId=" + _this.classificationId);
                    }
