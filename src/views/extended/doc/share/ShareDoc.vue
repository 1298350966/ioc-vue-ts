<template>
    <div>
        <div class="header-row" >
            <span class="header-title">共享文件库</span>
        </div>
        <div class="search-row">
            <el-form :inline="true" v-model="queryCondition">
        ...
        <el-dialog
            ...
            width="50%"
        >
            <el-form :model="docObj" label-width="120px" ref="docObj" style="margin-left: 50px;margin-right: 80px">
                ...
                <el-form-item label="选择共享文件" class="custom-form-item">
                    <el-upload
                        class="upload-demo"
        ...
        <el-dialog
            ...
            width="40%"
        >
            <tag-selected :multiple="selectedMultiple" ref="tagSelectRef" v-if="tagHackSet"
        ...
        >
            <file-classification-selected :multiple="selectedMultiple" ref="classificationSelectedVisibleRef"
                                          :source="source"
                                          v-if="classificationSelectedHackSet"></file-classification-selected>
            <span slot="footer" class="dialog-footer">
        ...
        <el-dialog
            ...
            width="45%"
        >
            <tenant-selected v-if="backListSelectedHackSet" :multiple="true" ref="tenantSelectedRef"></tenant-selected>
            <span slot="footer" class="dialog-footer">
                <el-button @click="backListVisible = false">取 消</el-button>
                <el-button type="primary" @click="sureSelectedBackList">确 定</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
<script>
    ...
    export default {
        components: {TagSelected, TenantSelected, FileClassificationSelected},
        data() {
            return {
                source: "0",
        ...
        methods: {
            ...
            listData() {
                ...
                this.tableData = [];
                this.loading = true;
                this.$Get(this.khConfig.api.queryShareDocForPlatForm, this.queryCondition).then(res => {
                    this.loading = false;
                    if (res.errCode === 0) {
            ...
            showBackListSelect() {
                ...
                _this.backListSelectedHackSet = false;
                _this.$nextTick(() => {
                    _this.backListSelectedHackSet = true;
                });
            },
            sureSelectedBackList() {
                let _this = this;
            ...
            handleEdit(index, row) {
                let _this = this;
                _this.dialogTitle = "编辑共享文件";
                _this.fileExtendId = row.id;
                _this.oldFileId = row.fileId;
                _this.isAdd = false;
                _this.fileList = [];
                _this.docFileList = [];
                _this.$Get(_this.khConfig.api.queryFileTenantBlackLisByFileId + "/" + row.fileId).then(res => {
                    if (res.errCode === 0) {
                        _this.docObj.fileId = row.fileId;
                        _this.docObj.id = row.id;
                        _this.docObj.templateName = row.templateName;
