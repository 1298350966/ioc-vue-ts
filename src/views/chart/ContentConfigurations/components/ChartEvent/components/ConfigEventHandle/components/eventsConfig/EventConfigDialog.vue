            ...
            <el-main>
              ...
              <el-tabs v-model="eventActiveName" class="demo-tabs">
                ...
                <el-tab-pane label="过滤条件" name="2" v-if="eventConfig.filters">
                  <FilterConfig :filters="eventConfig.filters" :mappingFieldOptions="mappingFieldOptions"></FilterConfig>
                </el-tab-pane>
              </el-tabs>
            </el-main>
          </el-container>
...
import { getComponentEvents } from "@/packages";
...
const eventsConfig = computed(() => {
  let Events = getComponentEvents(targetData.value.key)
  return Events
})
...
const mappingFieldOptions = computed(() => {
  if (eventDes.value) {
    eventDes.value.setVars && eventDes.value.setVars(targetData.value)
    return eventDes.value.vars
  } else { 
    return []
  }
}) 
