      ...
      <div class="go-mr-3">
        <div>
          <setting-item-box name="主体信息">
            <setting-item name="接口名称">
              <el-input size="small" :placeholder="targetData?.dataPondName || '暂无'" :disabled="true"> </el-input>
            </setting-item>
            <setting-item name="接口类型">
              <el-input size="small" :placeholder="requestHttpType || '暂无'" :disabled="true"></el-input>
            </setting-item>
          </setting-item-box>

          <setting-item-box>
            <setting-item name="组件间隔">
              <el-input size="small" :placeholder="`${requestInterval || '暂无'}`" :disabled="true">
                <template #suffix>
            ...
            <setting-item name="全局间隔（默认）">
              <el-input size="small" :placeholder="`${globalData?.requestInterval || '暂无'}`" :disabled="true">
          ...
          <setting-item-box name="源地址" :alone="true">
            <el-input size="small" :placeholder="globalData?.requestOriginUrl || '暂无'" :disabled="true">
              <template #prefix>
        ...
        <setting-item-box name="类型">
          ...
          <setting-item name="body 类型" v-if="requestContentType === RequestContentTypeEnum.DEFAULT">
            <el-input size="small" :placeholder="targetData && requestParamsBodyType" :disabled="true"></el-input>
          </setting-item>
        </setting-item-box>
        <div v-if="requestContentType === RequestContentTypeEnum.DEFAULT">
          <el-tabs v-model="tabValue">
            <el-tab-pane  v-for="item in RequestParamsTypeEnum" :key="item" :name="item" :label="item"> {{ item }} </el-tab-pane >
          </el-tabs>
          <!-- 各个页面 -->
          <div class="go-mt-3">
            ...
            <div v-else>
              <!-- 为 none 时 -->
              <el-card class="go-mt-3 go-pb-3" v-if="requestParamsBodyType === RequestBodyEnum['NONE']">
