<template>
  <template v-if="targetData.filter">
    <el-card shadow="never">
      ...
      <!-- 函数体 -->
      <div class="go-ml-4">
        <h-code :code="targetData.filter" language="typescript"></h-code>
      </div>
      <p>}</p>
      <el-space alignment="end">
        <el-button type="primary" tertiary size="small" @click="addFilter">
          编辑
        </el-button>
        <el-button tertiary size="small" @click="delFilter"> 删除 </el-button>
      </el-space>
    </el-card>
  </template>
  <template v-else>
