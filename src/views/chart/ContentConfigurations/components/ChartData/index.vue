<template>
  <div class="go-chart-configurations-data" v-if="targetData">
    <setting-item-box name="请求方式" :alone="true">
      <el-select-v2 v-model="targetData.request.requestDataType" :disabled="isNotData" :options="selectOptions" />
    </setting-item-box>
    <!-- 静态 -->
...
const ChartDataAjax = loadAsyncComponent(
  () => import("./components/ChartDataAjax/index.vue")
);
const ChartDataPond = loadAsyncComponent(
  () => import("./components/ChartDataPond/index.vue")
);
...
const isNotData = computed(() => {
  return typeof (targetData.value?.option?.dataset || targetData.value.data) === "undefined";
});
</script>
