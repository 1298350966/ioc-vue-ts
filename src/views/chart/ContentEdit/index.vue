<template>
  <EditRule></EditRule>
  <content-box id="chart-edit-layout" :flex="true" :showTop="false" :showBottom="true"
    @mousedown="mousedownHandleUnStop" @drop="dragHandle" @dragover.prevent="dragoverHandle"
      ...
      <edit-range>
        <div :style="{
          ...
          <div v-for="(item, index) in chartEditStore.getComponentList" :key="item.id">
            ...
            <!-- 单组件 -->
            <edit-shape-box v-else :data-id="item.id" :index="index" :style="useComponentStyle(item.attr, index)"
              ...
              <component class="edit-content-chart" :class="animationsClass(item.styles.animations)"
                :is="item.chartConfig.chartKey" :chartConfig="item" :themeSetting="themeSetting" :themeColor="themeColor"
                :style="{
