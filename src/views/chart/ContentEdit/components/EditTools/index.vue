<template>
  <div
    class="chart-edit-tools"
    :class="[
      settingStore.getChartToolsStatus,
      isMiniComputed ? 'isMini' : 'unMini',
    ]"
  ...
  >
    ...
    <el-icon
      v-show="
        settingStore.getChartToolsStatus === ToolsStatusEnum.ASIDE &&
        isMiniComputed
      "
      class="asideLogo"
      size="20px"
    ...
    >
      <div class="btn-item">
        <el-button
          ...
          @click="item.handle"
        >
          <el-icon size="20" v-if="isAside">
            <component :is="item.icon"></component>
          </el-icon>
          <component v-else :is="item.icon"></component>
          <span v-show="!isAside">{{ item.name }}</span>
        </el-button>
        <!-- 下载 -->
        <el-upload
          ...
          :http-request="importCustomRequest"
          :before-upload="importBeforeUpload"
        >
          <el-button :circle="isAside" secondary>
            <el-icon size="20" v-if="isAside">
              <component :is="item.icon"></component>
            </el-icon>
            <component v-else :is="item.icon"></component>
...
const { DownloadIcon, ShareIcon, PawIcon, SettingsSharpIcon, CreateIcon } =
  icon.ionicons5;
...
// 文件上传
const { importUploadFileListRef, importCustomRequest, importBeforeUpload } =
  useFile();
