<template>
  <dragDialog  custom-class="FunEditorDialog" v-model="dragDialogShow" title="添加变量" >
...
const emit = defineEmits(["modelValue:data"])
...
let ruleForm = reactive(createEmptyModel());
...
const save = () => {
  ruleFormRef.value.validate((valid)=>{
    if (valid) {
      ...
      addGlobalVar(cloneDeep(ruleForm))
      closeDialog()
...
const addEntityItem = () =>{
  ruleForm.entitys.push(createEntity()) 
}

const deleteEntityItem = (index) => {
  ruleForm.entitys.splice(index,1)
}
