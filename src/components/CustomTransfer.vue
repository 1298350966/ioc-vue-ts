<template>
    <div class="ty-transfer mt20 ml20">
        <div style="padding-bottom: 10px" v-if="custom">
            表头位置：
            起始行：
            <el-input-number v-model="startObj.startRow" :min="1" label="行"></el-input-number>
            起始列：
            <el-input-number v-model="startObj.startColumn" :min="1"  label="列"></el-input-number>
         <!-- <template v-if="isExport">
            <span style="margin-left: 30px">列明规则：</span>
              <el-radio v-model="startObj.exportRules" label="name">按名称</el-radio>
              <el-radio v-model="startObj.exportRules" label="order">按顺序</el-radio>
            </template>-->
        </div>
      <el-row v-if="isExport">
        <el-col :span="12" >
          <template >
            <el-upload
              class="upload-demo"
              ...
              <div slot="tip" class="el-upload__tip">上传导出模板</div>
            </el-upload>
          </template>
        </el-col>
      ...
      <el-row></el-row>
                    ...
                    <li v-for="item in sourceList">
                        <div class="ty-tree-div">
                            <label class="tyue-checkbox-wrapper">
                                ...
                                <span class="tyue-checkbox-txt" :title="item.name">{{item.name}}</span>
                            </label>
                        </div>
                    </li>
                </ul>
        ...
        <div v-if="custom" class="fl ty-transfer-list transfer-list-right" >
            <div class="ty-transfer-list-head">
                <div class="tyc-check-blue dib ml10">
                    <span>映射报表列名</span>
                </div>
                ...
                <ul class="ty-tree-select">
                    <li v-for="item in targetList">
                        <div class="ty-tree-div">
                            <label class="tyue-checkbox-wrapper">
                                  <span class="tyue-checkbox">
                                      <input type="string" v-model="item.excelName" class="tyue-checkbox-input">
        ...
        <div class="dialog-footer" style="margin-top: 15px;overflow: hidden;text-align: center">
          <el-button type="primary" @click.native="submitDataToParent">保存</el-button>
        </div>
    </div>
</template>
...
<script>
  import CustomUpload from "@/components/CustomUpload";
    export default {
        ...
        // targetdata: 目标数据集合
        // targettitle 目标数据标题
        // custom: 高级设置中报表导入导出设置的特殊处理
        // startObj: 高级设置中报表导入导出设置的起始行,起始列数据对象
      components: {
        CustomUpload
      },
        props: ['sourcedata','sourcetitle','targetdata','targettitle','custom','startObj','isExport',"fileList"],
        watch: {
            targetdata() {
                var source=[];
  ...
  /*          if(this.targetdata===null||this.targetdata.length===0){
        ...
        methods: {
            submitDataToParent(){
                ...
                this.$emit("submitDataToParent", this.targetList,this.startObj);
            },
            formatData(dataList) {
          ...
          handleRemove(file, fileList) {
            ...
            this.startObj.exportModelInfo.fileId = "";
            this.startObj.exportModelInfo.originalfile = "";
          },
          handlePreview(file){
          ...
          uploadCall(response, file, fileList) {
            if (response.errCode === 0 && response.datas != null && response.datas.length > 0) {
              ...
              this.startObj.exportModelInfo.fileId = response.datas[0].fileId;
              this.startObj.exportModelInfo.originalfile = response.datas[0].originalfile;
...
<style scoped>
 ...
 /*   .mt20{margin-top:13px;}*/
    ...
    .tyue-checkbox-txt.active{color: #00A0E8;}

    /*过滤*/
    .ty-transfer-list{width:200px;height:350px;border:1px solid #e8e8e8;border-radius:3px;background:#fff;box-shadow: 0 0 5px #e8e8e8}
    .ty-transfer-list-head{height:32px;line-height:32px;font-size:14px;color:#666;background-color: #f5f7fc;}
    ...
    .ty-tree-select{margin-top: 10px;}

    .ty-transfer-list-serach-icon{ width:17px;height:17px;display:block;position:absolute;top:4px;left:22px;z-index:10;}
    .ty-transfer-list-body{border-top:1px solid #e8e8e8;padding:4px 10px;height:318px;overflow-y:auto;box-sizing:border-box;}
    ...
    .ty-transfer .ty-tree-div{height:30px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;}
    ...
    .ty-transfer-operation{padding:107px 5px 0 5px;}
    ...
    .ty-transfer-btn-toright,.ty-transfer-btn-toright.disabled:hover{background-position:8px 2px;}

    .ty-transfer-btn-toright:hover,.ty-transfer-btn-toright.active,.ty-transfer-btn-toright.active:hover{background-position:8px -23px; }

    .ty-transfer-btn-toleft,.ty-transfer-btn-toleft.disabled:hover{background-position:6px -54px;}
    .ty-transfer-btn-toleft:hover,.ty-transfer-btn-toleft.active,.ty-transfer-btn-toleft.active:hover{background-position:6px -79px;}
    ...
    .ty-transfer-btn-totop, .ty-transfer-btn-totop.disabled:hover{background-position: 8px 2px;transform: rotate(-90deg);}
    .ty-transfer-btn-tobottom, .ty-transfer-btn-tobottom.disabled:hover{background-position: 8px 2px;transform: rotate(90deg);}
    ...
    .transfer-all-check{vertical-align: middle;}
    ...
    .four-icon.ty-transfer-operation{padding-top: 70px;}
    .ty-transfer-operation .ty-transfer-btn-zhiding,
    .ty-transfer-operation .ty-transfer-btn-zhidi{background-image: url(../assets/images/zhiding-zhidi.png);background-size: 27px 108px;background-repeat: no-repeat;}
    .ty-transfer-operation .ty-transfer-btn-zhiding:hover,
    .ty-transfer-operation .ty-transfer-btn-zhiding.active,
    .ty-transfer-operation .ty-transfer-btn-zhiding.active:hover{background-position: 4px -50px;}
    .ty-transfer-operation .ty-transfer-btn-zhidi:hover,
    .ty-transfer-operation .ty-transfer-btn-zhidi.active,
    .ty-transfer-operation .ty-transfer-btn-zhidi.active:hover{background-position: 4px -78px;}
    .ty-transfer-operation .ty-transfer-btn-zhiding.normal{background-position:4px 4px;}
    .ty-transfer-operation .ty-transfer-btn-zhidi.normal{background-position:4px -25px;}
    .ty-transfer-operation .ty-transfer-btn-zhiding.normal.active,
    .ty-transfer-operation .ty-transfer-btn-zhiding.normal:hover{background-position: 4px -50px;}
    .ty-transfer-operation .ty-transfer-btn-zhidi.normal.active,
    .ty-transfer-operation .ty-transfer-btn-zhidi.normal:hover {background-position:4px -78px;}
