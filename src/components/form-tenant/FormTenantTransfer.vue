    ...
    <div class="div-center">
        <div class="ty-transfer mt20 ml20 transfer-center" >
            ...
            <div class="fl ty-transfer-list transfer-list-left transfer-left"  >
                <div class="ty-transfer-list-body">
                    <ul class="ty-tree-select">
                        <li v-for="item in allTenants">
                            <div class="ty-tree-div" >
                                <label class="tyue-checkbox-wrapper" @change="addTenant(item)">
                                    ...
                                    <i class="el-icon-s-custom"  style="color: #409EFF;font-size: 18px" ></i>
                                    <span class="tyue-checkbox-txt">{{item.tenantName}}</span>
                                </label>
                            </div>
                        </li>
                    </ul>
            ...
            <div class="fl ty-transfer-list transfer-list-right transfer-left" style="margin-left: 50px">
                <div class="ty-transfer-list-body">
                    <ul class="ty-tree-select">
                        <li v-for="item in selectTenants">
                            <div class="ty-tree-div">
...
<script>
    export default {
        ...
        methods:{
            queryTenant(){
                let queryTenantParam = {
                    pageNum:1,
                    pageSize:1000
                }
                this.$Get(this.khConfig.api.listTenant, queryTenantParam).then(res => {
                    if (res.errCode === 0) {
                        if (!this.KHUtils.isNull(res.datas) && res.datas.length > 0) {
                                this.allTenants =[];
                                this.allTenants = res.datas;
                                this.initSelect(this.allTenants)
                                this.checkSelect();
                        }
                    }
                });
            ...
            listTenantPression(){
                this.formPermissions = [];
                this.$Get(this.khConfig.api.queryFormTenantPression+"/"+this.formId).then(res => {
                    if (res.errCode === 0) {
                        if (!this.KHUtils.isNull(res.datas) && res.datas.length > 0) {
                            this.formPermissions = res.datas;
            ...
            initCheckSelect(){
                ...
                if (this.formPermissions != null && this.formPermissions.length > 0) {
                    for (var i = 0; i < this.formPermissions.length; i++) {
                        ...
                        let newObj = {};
                        newObj.id = obj.id;
                        newObj.tenantName = obj.tenantName;
                        this.selectTenants.push(newObj);
                    }
                    this.checkSelect();
                }
            },
            ...
            addTenant(item){
                if (item.isSelected) {
                    let newObj = {};
                    newObj.id = item.id;
                    newObj.tenantName = item.tenantName;
                    this.selectTenants.push(newObj);

                } else {
                    if (this.selectTenants != null && this.selectTenants.length > 0) {
                        for (var i = 0; i < this.selectTenants.length; i++) {
                            let tenant = this.selectTenants[i];
                            if (tenant.id === item.id) {
                                this.selectTenants.splice(i, 1);
                            }
                        }
                    }
                }
            ...
            save(){
                this.$emit("formTenantsAllot",this.formId, this.selectTenants);
            },
            //校验是选中状态
            checkSelect(){
                for (var i = 0;i<this.allTenants.length;i++){
                    let obj = this.allTenants[i];
                    obj.isSelected = false;
                    if(this.selectTenants != null && this.selectTenants.length>0){
                        for(var j = 0;j<this.selectTenants.length;j++){
                            let selectObj = this.selectTenants[j];
