    ...
    <section>
        <div class="el-input">
            <input type="text"
                   v-model="filterText"
                   autocomplete="off"
                   placeholder="输入类目名称搜索"
                   class="el-input__inner" style="width: 200px">
            <el-button type="primary" @click="handleAddDicCatalog" style="margin-left:15px;">新增</el-button>
        </div>
        <el-tree
            :data="dicCatalogData"
            default-expand-all
...
<script>
    export default {
        name: "DicCatalogSelect",
        props: {
            "tenantId": {required: false}, // 租户ID
            "typeCode": {type: String, required: true}, // 字典类型
        ...
        methods: {
            ...
            selectDicCatalog(data) {
                ...
                if (data.level !== 3) {
                    ...
                    _this.$message.warning("此为三级类目 ,不能作为上级类目选择");
                    _this.display = 'block'
                }
            },
            // 显示字典类目的编辑界面
            ...
            saveDicCatalog() {
                ...
                function convertDicCatalogForm2Array(_form, _this) {
                    ...
                    paramArray.push("typeCode=" + _this.typeCode);
