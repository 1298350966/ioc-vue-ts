<template>
    <el-row style="height: 100%" class="menu-setting">
        ...
        <el-dialog title="新增目录" :visible.sync="addMenuVisible" :close-on-click-modal="false" :modal="false"    custom-class="advanced-set-dialog">
            <el-form :model="addMenuForm" label-width="100px" :rules="addFormRules" ref="addMenuForm" style="margin-top:20px;color: black;padding:0 25px;" class="custom-box1">
                <el-row>
                    <el-col :span="12">
                    ...
                    <el-col :span="12">
                        <el-form-item label="上级目录">
                            <el-input style="line-height: 30px;" v-model="addMenuForm.parentMenuName"
                                      @focus="displayTree" clearable @clear="handleClearInput">
                                <!-- {{addMenuForm.parentMenuName}} -->
                            </el-input>
                            <el-tree
                                :data="menuTree"
                                :props="props"
                                :style="{zIndex: 2, display: display,maxHeight:'40vh'}"
                                node-key="id"
                                accordion
                                highlight-current
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //解决输入框点击clear时this.addMenuForm的parentMenuName属性已经改变，但视图没有更新
            handleClearInput(){
                ...
                _this.$confirm('确认保存菜单配置吗？', '提示', {}).then(() => {
                    if (_this.prepareAddMenu.length > 0) {
                        _this.prepareAddMenu.forEach((menu, index) => {
                            ...
                            console.log(menu, _this.convertForm2Array(menu));
                            this.$PostFromData(_this.khConfig.api.addMenu, _this.convertForm2Array(menu))
                                .then(res => {
                                    if (res.errCode === 0) {
                                        this.$notify({
                                            title: '成功',
            ...
            remove(node, data) {
                ...
                _this.$confirm('确认移除此菜单配置吗？', '提示', {}).then(() => {
                    ...
                    if(!data.hasOwnProperty('id')) return;  //如果删除的是刚刚添加尚未保存的菜单，则无需调删除接口
            ...
            convertForm2Array(_form) {
                let paramArray = {};
                paramArray.menuType= "1"
                paramArray.formId= _form.formId
