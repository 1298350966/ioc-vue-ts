    ...
    <div class="ty-transfer mt20 ml20 transfer-center" >
      ...
      <div class="fl ty-transfer-list transfer-list-left transfer-left"  >
        <div class="ty-transfer-list-head">
          ...
          <!--<div class="dib fs14 fw ml5">部门</div>-->
          <el-breadcrumb class="dib fs14 fw ml5" separator-class="el-icon-arrow-right" style="margin:5px;"  >
            <el-breadcrumb-item v-for="(item,key) in titleList" :key="key" style="margin-top: 5px"><a @click="backOrg(item)">{{item.name}}</a></el-breadcrumb-item>
          </el-breadcrumb>
        </div>
        <div class="ty-transfer-list-body">
          <ul class="ty-tree-select">
            <li v-for="item in currentOrgUser">
              <div class="ty-tree-div" >
                ...
                <el-link type="primary" style="float: right" :disabled="isSelect(item)" v-if="isOrg(item)"
      ...
      <div class="fl ty-transfer-list transfer-list-right transfer-left" style="margin-left: 50px">
        ...
        <div class="ty-transfer-list-body">
          <ul class="ty-tree-select">
            <li v-for="item in selectOrgUser">
              <div class="ty-tree-div">
...
<script>
  ...
  export default {
    ...
    props: ['formPermissions','tenantId'],
    watch:{
      formPermissions(){
    ...
    methods: {
      initDeptUserTree() {
        ...
        this.selectOrgUser = [];
        this.titleList = [];
        this.queryOrgUser(null,0);
      },
      //按部门id查询组织架构下的部门和用户 type:0 初始化第一次 1：进入部门 2：返回
        queryOrgUser(orgId,type,item){
          let param = {
              orgId:orgId,
              tenantId:this.tenantId
          }
            this.$Get(this.khConfig.api.listTenantOrgStructure,param)
                .then(res => {
                    if (res.errCode === 0) {
                        console.log("--"+JSON.stringify(res))
                        this.currentOrgUser = res.list;
      ...
      exterOrg(item){
        this.currentOrgUser = [];
        this.queryOrgUser(item.id,1,item);
      },
      //返回到某个目录
      backOrg(item){
        this.currentOrgUser = [];
        if(item.id == -1){
            //首页
            this.queryOrgUser(null,2,item);
        }else {
            this.queryOrgUser(item.id,2,item);
        }
      },
      initCheckSelect(){
        this.selectOrgUser = [];
        if (this.formPermissions != null && this.formPermissions.length > 0) {
          for (var i = 0; i < this.formPermissions.length; i++) {
            ...
            let newObj = {};
            newObj.id = parseInt(obj.subId);
            newObj.name = obj.subName;
      ...
      save(){
        this.$emit("getCheckedNodesFromOrgUserTree", this.selectOrgUser);
      },
