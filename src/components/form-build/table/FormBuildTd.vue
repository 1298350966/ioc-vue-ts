<template>
    <div>
        <el-form :class="{formBox:this.data.config.isBorder}" ref="buildForm"
                 label-suffix=":"
            ...
            <template v-for="item in data.list">
                <template v-if="item.type === 'grid' || item.type === 'table' || item.type === 'flow_table'">
                    <el-row
                        ...
                        <el-col  class="lineItem" v-for="(col, colIndex) in item.columns" :key="colIndex"
                                :span="col.span">
                            <div class="cellBox" v-for="(colItem,index) in col.list" :key="index">
                                <form-build-item
                                    ...
                                    <span class="lineCell" :style="lineCellStyle"></span>
                            </div>
                        </el-col>
                    </el-row>
                </template>
                <div class="cellBox" v-else>
                    <form-build-item
                        class="line"
...
<script>
    import FormBuildItem from '@/components/form-build/FormBuildItem';
    
    export default {
        name: 'FormBuildTd',
        components: {
            FormBuildItem     // 子表单批量提交时渲染
        },
        // data: 表单配置; (必填)
        ...
        computed: {
            lineCellStyle(){
                return {left: this.data ? (this.data.config.labelWidth + 'px') : '100px'}
            }
        },
        methods: {
