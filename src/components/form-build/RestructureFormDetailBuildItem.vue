<template>
  ...
  <el-form-item class="formBuildItem" :label="box.type !== 'text' ? box.name + ' : ' : ''" :prop="box.model">
    ...
    <template v-if="box.type === 'radio'">
      {{ getSelectRadioLable(dataModel,box) }}
    </template>
    ...
    <template v-if="box.type === 'signature'">
      <el-input
        v-model="dataModel"
        :hidden="true"
        style="width: 1px; float: left"
        v-if="box.options.required"
      ></el-input>
    ...
    <template v-if="box.type === 'file_upload' || box.type === 'image_upload'">
      ...
      <div
        v-if="!addOrEdit && box.type === 'file_upload' && dataModel instanceof Array"
        style="width: 50%"
      >
        <ul class="el-upload-list el-upload-list--text">
          ...
          >
            ...
            <el-col :span="21">
              ...
              <p
                style="font-size: 10px; position: relative; bottom: 15px"
                v-if="
                  file.fileSuffix &&
    ...
    <template v-if="box.type === 'cascade'">
      ...
      <template v-if="box.type === 'm_data_ref'">
           <m-data-ref-widget :readOnly="true" :formId="formId" :dataModel.sync="dataModel" :box="box"></m-data-ref-widget>
      </template>
      <template v-if="box.type === 'calculate_formula'">
          {{getCalculateFormulaValue}}
      </template>
      <preview-image-dialog :visible.sync="imageDialogVisible" :imageId="imageId" v-if="imageDialogVisible"></preview-image-dialog>
  </el-form-item>
</template>
...
export default {
  ...
  props: ["box", "models", "rules", "addOrEdit", "remote", "formId", "isDetail"],
  /*        props: {
  ...
  created() {
    ...
    if (_this.box.type === "file_upload") {
      if (!_this.KHUtils.isNull(_this.dataModel)) {
        ...
        _this.dataModel.forEach((obj) => {
          ...
          xhr.open(
            "GET",
            _this.khConfig.host.rest + _this.khConfig.api.getFileInfo + "/" + obj,
            false
          );
    ...
    if (_this.box.type === "gis_select") {
      ...
      if (
        !_this.KHUtils.isNull(_this.box.options.lat) &&
        !_this.KHUtils.isNull(_this.box.options.lnt)
      ) {
    ...
    if (_this.box.type === "company_select") {
      ...
      if (!_this.KHUtils.isNull(userType) && userType === "1") {
        ...
        _this.$Get(_this.khConfig.api.getCompanyInfoByUserId, {}).then((res) => {
          if (res.errCode === 0) {
            _this.dataModel = {};
  ...
  computed: {
    ...
    getSelectValue() {
      return function (selectValue) {
        ...
        let array = selectValue ? JSON.parse(selectValue) : "";
        if (array instanceof Array && array.length > 0) {
          array.forEach((item, index) => {
            value = value.concat(item.v, "、");
          });
        }
        return value;
      };
    ...
    //通过value获取单选控件，下拉控件的label值
    getSelectRadioLable(){
      return function (value,box) {
        let options =  box.options.options;
        if(box.type =='radio' || box.type == 'select'){
        ...
        if(box.type == 'checkbox' || box.type == 'multiple_select'){
          let label = [];
  ...
  methods: {
    ...
    deleteFile(fileId) {
      ...
      const index = this.fileObj[this.box.model].findIndex(
            ...
            if (flag) {
              if (this.box.type === "image_upload" || this.box.type === "image_upload") {
                this.models[this.box.model].push(
                  this.khConfig.host.rest + this.khConfig.api.imageDownload + "/" + fileId
                );
              } else {
...
@font-face {
  ...
  src: url("//at.alicdn.com/t/font_1467856_gfac3yoy5zk.eot?#iefix")
