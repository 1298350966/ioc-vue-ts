            ...
            <template v-for="(item,index) in data.list">
                <template v-if="item.type === 'grid' || item.type === 'table' || item.type === 'flow_table'">
                    <el-row
                        ...
                        <el-col class="lineItem"  v-for="(col, colIndex) in item.columns" :key="colIndex" :span="col.span">
                ...
                <template v-else>
                    <div class="cellBox" :key="index"  v-if="item.show && (addOrEdit ? (Object.keys(item.options).indexOf('showFields') < 0 || !item.options.showFields) :  (Object.keys(item.options).indexOf('editShowFields') < 0 || !item.options.editShowFields))">
                        <form-build-item
                            class="line"
                            :key="item.model" 
                            :models.sync="models" 
...
<script>
    ...
    export default {
        ...
        props: ['data', 'value', 'addOrEdit', 'remote', "formId","allCols"],
        /*        props: {
