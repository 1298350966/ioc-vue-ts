    ...
    <div id="myDiv" ref="myDiv">
        <el-form ref="buildForm"
            ...
            <template v-for="item in data.list">
                    ...
                    <el-row
                        ...
                        <el-col v-if="item.type === 'grid'" v-for="(col, colIndex) in item.columns" :key="colIndex"
                            ...
                            <template v-for="colItem in col.list">
                                <!--查看审批,相当于编辑-->
                                <form-build-item-process
                                    v-if="colItem.show && (Object.keys(colItem.options).indexOf('editShowFields') < 0 || !colItem.options.editShowFields)"
                                    :key="colItem.model" :models.sync="models" :formId="formId"
                                    :rules="rules" :box="colItem"
                                    @func="getMsgFormSon"></form-build-item-process>
                            </template>
                        </el-col>
                    </el-row>
                </template>
                <template v-else>
                    <form-build-item-process
                        v-if="item.show && (Object.keys(item.options).indexOf('editShowFields') < 0 || !item.options.editShowFields)"
...
<script>
    import FormBuildItemProcess from '@/components/form-build/process/RestructureFormBuildItemProcess';

    export default {
        name: 'RestructureFormBuildProcess',  // 审批记录查看时渲染
        components: {
            FormBuildItemProcess
        },
        // data: 表单配置; (必填)
        // value: 表单填写的值{input_01:1,text_01:"111"}; (必填)
        props: ['data', 'value','formId'],
