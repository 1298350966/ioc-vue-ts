<template>
    ...
    <el-form-item :label="box.type!=='text' ? box.name : ''" :prop="box.model">
        ...
        <template v-if="box.type === 'user_select' || box.type === 'm_user_select'">
            ...
            <el-dialog :title="box.options.maps.title" :visible.sync="selectDialogVisible"
                       ...
                       <el-row v-if="box.options.displayModel === 'table' && colsArray.length > 0">
                            <userSelectWidget 
                                :widgetType="box.type" 
                                :dataModel="box.type==='user_select'?dataModel[box.label_model]:dataModel"
        ...
        <template
            v-if="box.type === 'm_org_select'  || box.type === 'm_company_select'|| box.type === 'm_role_select' || box.type === 'm_dic_select' ">
            <el-input
                @focus="loadSysSelectData"
                clearable
                @clear="clearDataModel"
            ...
            <el-dialog :title="box.options.maps.title" :visible.sync="selectDialogVisible"
                ...
                <el-row v-if="box.options.displayModel === 'table' && colsArray.length > 0">
                    <el-input style="width: 160px;" :placeholder="box.type=== 'm_user_select' ? '用户名' :box.label_name"
                              prefix-icon="el-icon-search" v-model="keywords" @blur="getDataSource"></el-input>
                    <el-table
                        :data="tableData"
                        v-loading="tableDataLoading"
        ...
        <template v-if="box.type === 'radio'">
            <el-radio-group v-model="dataModel"
                            :style="{width: box.options.width}"
                            :disabled="box.options.disabled" @change="controlShow">
                <el-radio
        ...
        <template v-if="box.type === 'file_upload' || box.type === 'image_upload'">
            ...
            <div v-if="!addOrEdit && box.type === 'file_upload' && dataModel instanceof Array" style="width: 100%">
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            // 通用数据选择控件
            if (_this.box.type === 'data_select' ||  _this.box.type === 'm_data_select' ||_this.box.type === 'org_select'  || _this.box.type === 'user_select' || _this.box.type === 'm_user_select' || _this.box.type === 'company_select'  || _this.box.type === 'm_company_select' || _this.box.type === 'role_select' || _this.box.type === 'm_role_select'  || _this.box.type === 'dic_select' || _this.box.type === 'm_dic_select') {
        ...
        methods: {
          ...
          isObject(object){
            ...
            clearDataModel() {
                ...
                if(this.box.type ===  'm_data_select' || this.box.type === 'm_org_select'  || this.box.type === 'm_user_select'  || this.box.type === 'm_company_select'  || this.box.type === 'm_role_select'  || this.box.type === 'm_dic_select'){
                    _this.dataModel = ''
                }else{
                ...
                //由于对应数据非响应式，导致清空数据视图没有刷新，所以此处使用强制刷新
                _this.$forceUpdate();
            },
            // 组织,用户等控件加载选择项数据
            loadSysSelectData() {
                ...
                if (_this.box.options.displayModel === 'table') {
                    ...
                    let param = {};
                    if (_this.box.type === 'user_select'|| _this.box.type === 'm_user_select' || _this.box.type === 'company_select' || _this.box.type === 'm_company_select') {
                    ...
                    this.$Get(_this.box.options.remoteUrl, param).then(res => {
                        if (res.errCode === 0) {
                            if (_this.box.type === 'data_select' || _this.box.type === 'm_data_select' || _this.box.type === 'company_select' || _this.box.type === 'm_company_select') {
            ...
            loadNode(node, resolve) {
                ...
                if (this.box.type === 'data_select' || this.box.type === 'm_data_select' || this.box.type === 'user_select' || this.box.type === 'm_user_select' || this.box.type === 'role_select' || this.box.type === 'm_role_select' || this.box.type === 'company_select' || this.box.type === 'm_company_select') {
                                                ...
                                                if (selected[obj.selectKey] instanceof Array && selected[obj.selectKey].length > 0) {
                                                    ...
                                                    if(col){
                                                        if (col.type == "org_select") {
                                                            //默认取第一个
                                                            let data = array[0];
                                                            _this.dataModel[col.model] = data.id;
                                                            _this.dataModel[col.label_model] = data.orgName;
                                                            ...
                                                            _this.dataModel[obj.model] = JSON.stringify(JsopDataModel);

                                                        } else {
                                                            _this.dataModel[obj.model] = value;
                                                        }
                                                    }else {
                                                        _this.dataModel[obj.model] = value;
                                                    }
                                                ...
                                                if (selected[obj.selectKey] instanceof Array && selected[obj.selectKey].length > 0) {
                                                    ...
                                                    if(col){
                                                        if (col.type == "role_select") {
                                                            ...
                                                            let data = array[0];
                                                            _this.dataModel[col.model] = data.id;
                                                            _this.dataModel[col.label_model] = data.roleName;
