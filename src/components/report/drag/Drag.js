...
import $ from "jquery"
import windowResize from '@/components/report/drag/mixins/windowResize'
...
function init() {
    ...
    this.cellHeight = this.baseHeight + this.baseMarginTop;  // 设置单元格基础的高度
    positionBox = [];
    coordinates = []; //坐标点集合
    lastTask = undefined;
...
export default {
    ...
    methods: {
        ...
        startMove(e, item, index) {  
            ...
            $(window).mouseup(this.itemMouseUp)
         
            $(window).mousemove(this.itemMouseMove);
        },
        itemMouseMove(e) {
            if(e.buttons === 0){              
                return  this.itemMouseUp(e)
            }
            let vm = this
            let infoBox = this.infoBox;
            let moveItem = infoBox.moveItem;
            let resizeItem = infoBox.resizeItem;
            if (resizeItem) { //调整大小时
        ...
        itemMouseUp(e) {
            console.log("drag.itemMouseUp",e)
            let vm = this
            ...
            vm.infoBox = {};         
            $(window).off('mousemove', this.itemMouseMove);
            $(window).off('mouseup', this.itemMouseUp);
            this.$emit("onItemMouseUp")
        },
        endMove(e) {  
        },
        moving(e) {
