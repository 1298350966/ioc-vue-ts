        ...
        <div v-for="(item, index) in formList" :key="index" class="cardItem">
          <el-radio class="radio-box" :label="item" border size="medium">
            <div class="radio-label">
              ...
              <span class="text">{{ item.reportName }}</span>
            </div>
          </el-radio>
        </div>
      </el-radio-group>
...
export default {
    ...
    formList(newVl, oldVl) {
      if (newVl != oldVl) {
        let formObj = this.formList.find((value, index, array) => {
          return value.reportId == this.reportId;
        });
        if (formObj) {
  ...
  methods: {
    ...
    queryFormList() {
      let params = {
          reportName: this.formName || null,
        pageNum: this.pagination.page,
        pageSize: this.pagination.size,
      };
      this.formList = [];
      this.$Get(this.khConfig.api.queryReportConf, params).then((res) => {
        if (res.errCode === 0) {
          this.formList = res.dataReports;
