<template>
    <div class="ty-transfer mt20 ml20">
                ...
                <ul class="ty-tree-select">
                    <li v-for="item in targetList">
                        <div class="ty-tree-div">
                            ...
                            <a style="float: right" v-if="isResult && item.name !== '序号' && item.name !== '操作'"
                                  @click="jumpSet(item)" class="custom-a-jump">跳转配置</a>
                        </div>
        ...
        <el-dialog title="链接设置" :visible.sync="operationSettingsVisible" :modal="false">
            <div class="block">
                <operation-transfer :sourcedata="sourcedata" sourcetitle="请选择"
                                    :targetdata="jumpTargetdata" targettitle="已选择表单列名"
                                    @submitDataToParent="saveResultParam" :isOptBtn="false" :formList="formList"
                                    :start-obj="jumpStartObj"></operation-transfer>
            </div>
          <el-row>
            <el-col :span="24" style="font-size: 14px;font-family:”SimSun”,serif">
              <p style="margin: 5px 0;">配置说明</p>
              <p style="margin: 5px 0;">1、选择链接方式为组件的说明：填写的组件必须存在于项目当中，并将其实际对应的相对路径填写完毕。传递的参数名称必须与该组件中props定义的相同。比如预添加src/components/tree目录下的OrgUserTree组件，则相对路径填写为tree/OrgTree,参数的名称应填写对应组件中props定义的defaultCheckedKeys。</p>
              <p style="margin: 5px 0;">2、现平台支持的组件：关联的信息已列表形式显示 component: [ListFormCustom] - props: ['formId', 'listBusinessId']; 表单形式显示 component: [ViewFormCustom] - props: ['formId', 'businessId']。其中formId为必填项，是从配置中的关联表单属性选择,而其他参数通过穿梭框配置即可。</p>
...
<script>
    ...
    export default {
        ...
        props: ['sourcedata', 'sourcetitle', 'targetdata', 'targettitle', 'custom', 'startObj', 'isExport', "fileList", "isResult", "jumpInfoList","formList"],
        watch: {
            targetdata() {
                var source = [];
        ...
        data() {
            return {
                operationSettingsVisible: false,
                sourceList: [],
                targetList: [],
                exportFileList: [],
                jumpStartObj: {type: "0", urlType: "0"},
                jumpTargetdata: []
            }
        },
        computed: {
        ...
        methods: {
            submitDataToParent() {
                ...
                let _this = this;
                this.$emit("submitDataToParent", _this.targetList, _this.startObj);
            },
            formatData(dataList) {
            ...
            jumpSet(item) {
                let _this = this;
                let key = item.id;
                _this.jumpStartObj = {key: key, label: "", type: "0", urlType: "0", url: ""};
                _this.jumpTargetdata = [];
                let datas = _this.jumpInfoList;
                if (datas != null && datas.length > 0) {
                    datas.forEach(jumpInfo => {
                        if (jumpInfo.key === key) {
                            _this.jumpStartObj.label = jumpInfo.label;
                            _this.jumpStartObj.type = jumpInfo.type;
                            _this.jumpStartObj.url = jumpInfo.url;
                            _this.jumpStartObj.urlType = jumpInfo.urlType;
                            _this.jumpStartObj.formId = jumpInfo.formId;
                            _this.jumpTargetdata = [];
                            if (jumpInfo.params != null && jumpInfo.params instanceof Array) {
                                jumpInfo.params.forEach(item => {
                                    let newObj = {};
                                    newObj.id = item.model;
                                    newObj.name = item.name;
                                    newObj.isSelected = false;
                                    newObj.paramName = item.paramName;
                                    _this.jumpTargetdata.push(newObj);
                                })
                ...
                _this.operationSettingsVisible = true;

            },
            saveResultParam(targetList, obj, formId) {
                let index = this.jumpInfoList.findIndex(item => {
                    if (item.key === obj.key) {
                        return true
                    }
                });
                if (index !== -1) {
                    this.jumpInfoList.splice(index, 1);
                }
                let newObj = {key: "", label: "", type: "", urlType: "0", url: "", params: []};
                newObj.key = obj.key;
                newObj.label = obj.label;
                newObj.type = obj.type;
                newObj.url = obj.url;
                newObj.urlType = obj.urlType;
                ...
                if (targetList != null && targetList.length > 0) {
                    targetList.forEach(item => {
                        ...
                        newObj.params.push(param);
                    })
                }
                this.jumpInfoList.push(newObj);
...
<style scoped>
    ...
    a.custom-a-jump{color:#606266;text-decoration:none}
    a.custom-a-jump:hover{color:#409EFF;}
</style>
