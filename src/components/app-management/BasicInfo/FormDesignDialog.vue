<template>
    <el-row class="process-manage">
        <el-dialog  :modal="false" :title="formConf.formName === '' ? '基础设置' : formConf.formName"
                ...
                <el-row v-if="active === 0" style="padding: 40px;height: 100%;overflow: auto">
                    <el-form ref="formConf" :model="formConf" label-width="100px" :rules="formConfRules"
                        ...
                        <el-form-item label="模板图标">
                            ...
                            <p>
                                <img v-if="showImage" :src="imgSrc" alt=" " width="100px" height="100px"/>
                            </p>
                        </el-form-item>
        ...
        <form-group-add :visible="addFormGroupVisible"
                        ...
                        @changeFormGroupVisible="changeFormGroupVisible"
        ></form-group-add>
        <el-dialog title="表单权限" :visible.sync="formPermissionVisible" append-to-body width="50%">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 编辑
            handleEdit(row) {
                this.$Get(this.khConfig.api.getFormConf + "/" + row.formId).then((res) => {
                    if (res.errCode === 0) {
                        let myFormConf = res.formConf;
                        this.addOrEdit = false;
                        this.active = 0;
                        this.editFormVisible = true;
                        this.formConf = {
                            formId: myFormConf.formId,
                            formName: myFormConf.formName,
                            formGroup: myFormConf.formGroup,
                            formComment: myFormConf.formComment,
                        };
                        this.formConfig = JSON.parse(myFormConf.formMeta);
                        this.formId = myFormConf.formId;
                        if (!this.KHUtils.isNull(this.formId)) {
                            // 查询表单权限信息
                            this.listFormPermission(this.formId);
                        }
                        if (!this.KHUtils.isNull(myFormConf.formImg)) {
                            // 查询图标
                            this.imgSrc = this.khConfig.host.rest + this.khConfig.api.imageDownload + '/' +  myFormConf.formImg;
                            this.showImage = true;
                        }
                    }
                });
            ...
            next() {
                ...
                if (this.active === 0) {
                    ...
                    this.nextLoading = true;
                    this.notifySave++;
                    this.$emit("onFormId", this.formId);
                } else if (this.active === 3) {
                    // 通知高级设置组件保存
                    this.nextLoading = true;
                    this.notifySaveMenu++;
