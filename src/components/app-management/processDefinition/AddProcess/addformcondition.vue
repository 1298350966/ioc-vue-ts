<template>
  ...
  >
    <el-form label-width="auto">
      ...
      <el-form-item required v-if="conditions.type == 11">
              <el-select
                value-key="model"
                v-model="conditions.companyModel.model"
                placeholder="请选择"
      ...
      <el-form-item v-if="conditions.type == 10">
              <el-button type="primary" @click="companyRecipientVisible = true"
                >添加公司</el-button
              >
              <p class="selected_list">
                <span
                  v-for="(item, index) in conditions.company"
                  :key="index"
                  >{{ item.name }}
                  <img
               ...
               <el-dialog title="企业接收人" :visible.sync="companyRecipientVisible" v-if="companyRecipientVisible" width="50%" :modal="false" :append-to-body="true"
                   :close-on-click-modal="false">
                <company-transfer :selectCompany="conditions.company" @getSelectCompany="getSelectCompany"></company-transfer>
              </el-dialog>
  
      </el-form-item>
    </el-form>
    <fromConditionGroup :conditions.sync="conditions.conditions"></fromConditionGroup>
...
import fromConditionGroup from "@/components/app-management/processDefinition/AddProcess/fromConditionGroup";
...
export default {
  ...
  mounted() {
    ...
    if (this.data) {
      ...
      this.conditions.type = this.data.type || "0";
      ...
      this.conditions.conditions = this.data.conditions || [];
      this.conditions.company = this.data.company || [];
      this.conditions.companyModel =  this.conditions.companyModel ||{model:"",labelModel:""}
    }
  },
  methods: {
    changeCompanyModel(model){
      let company =  this.companySelect.find(value =>  value.model == model )
      if(!company) return this.$message.warning('未找到选择企业')
      this.conditions.companyModel.labelModel = company.label_model
      
    },
    getSelectCompany(company){
      this.conditions.company = company
    ...
    radioChange() {
      ...
      this.conditions.companyModel = {model:"",labelModel:""}
    },
  },
};
</script>
