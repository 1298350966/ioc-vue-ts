<template>
  <div class="automaticTransfer">
    <div class="setInfo">
      ...
      <p class="title">转交给谁</p>
      <el-form>
        <selectRemindedPerson
          v-if="setting.transfer"
          :recipient="setting.transfer"
        ></selectRemindedPerson>
      </el-form>

      <p class="title">提醒方式（提醒消息将会发给原处理人）</p>
      <p>
          ...
          <div class="addVariableBox">
            ...
            <div class="variable-name-box">
              <span
                ...
                :key="index"
                @click="
                  setting.content = setting.content + '${' + variable.model + '}'
                "
                >{{ variable.name }}</span
              >
            </div>
