  ...
  >
    <el-form label-width="auto">
      ...
      <el-form-item label="主管:" required v-if="conditions.type == 2">
        ...
        <p>找不到主管时，由上级主管 <i class="el-icon-warning-outline"></i></p>
      </el-form-item>
      ...
      <el-select v-model="conditions.dealingLeader" placeholder="请选择">
        <el-option
          v-for="item in executiveDirectorOptions"
          :key="item.value"
          :label="item.label"
      ...
      <el-form-item v-if="conditions.type == 11">
        <el-button type="primary" @click="dealingOrgAndRoleVisible = true"
          >添加</el-button
        >
        <el-table
          row-class-name="tableRowClassName"
          :data="conditions.dealingOrgAndRole"
          style="width: 100%"
        >
          <el-table-column prop="roleName" label="角色" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
              <el-button
                type="text"
                size="small"
                @click="conditions.dealingOrgAndRole.splice(scope.$index, 1)"
              >
                移除
              </el-button>
            </template>
        ...
        <selectDepartmentRole
          ...
          @onData="conditions.dealingOrgAndRole.push($event)"
        ></selectDepartmentRole>
      </el-form-item>
    </el-form>
...
export default {
  ...
  mounted() {
    ...
    if (this.data) {
      ...
      this.conditions.dealingLeade = this.data.dealingLeade || '';
      this.conditions.dealingOrgAndRole = this.data.dealingOrgAndRole || [];
    }
  },
  methods: {
    onData(e, data) {
    ...
    radioChange() {
      ...
      this.conditions.dealingLeade =  '';
      this.conditions.dealingOrgAndRole =  [];
    },
  },
};
</script>
